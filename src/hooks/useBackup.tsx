...
export const useBackup = () => {
  ...
  // Fetch saved backups
  const { data: savedBackups = [], isLoading: isLoadingBackups } = useQuery({
    queryKey: ['user-backups', user?.id],
    queryFn: async () => {
      if (!user) return [];
      ...
      return data as SavedBackup[];
    },
    enabled: !!user,
  });

  const generateBackupData = useCallback(async (): Promise<BackupData | null> => {
    if (!user) return null;
  ...
  const saveBackupToSystem = useCallback(async (name?: string): Promise<boolean> => {
    if (!user) {
    ...
    try {
      ...
      const backupName = name || `Backup ${format(new Date(), 'dd/MM/yyyy HH:mm')}`;
      ...
      queryClient.invalidateQueries({ queryKey: ['user-backups', user.id] });
  ...
  }, [user, t, generateBackupData, queryClient]);
  ...
  const restoreFromSavedBackup = useCallback(async (backupId: string): Promise<boolean> => {
    if (!user) {
    ...
    try {
      ...
      const backupData = backupRecord.backup_data as unknown as BackupData;
      return await processImport(backupData.data);
    } catch (error) {
  ...
  const processImport = useCallback(async (backupData: BackupData['data']): Promise<boolean> => {
  ...
  const importBackup = useCallback(async (file: File): Promise<boolean> => {
    ...
    try {
      ...
      return await processImport(data.data);
    } catch (error) {
  ...
  }, [user, t, validateBackup, processImport]);
