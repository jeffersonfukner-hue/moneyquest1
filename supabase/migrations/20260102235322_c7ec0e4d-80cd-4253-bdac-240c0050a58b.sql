-- Adicionar colunas para controle do trial premium
ALTER TABLE public.profiles 
  ADD COLUMN IF NOT EXISTS trial_start_date timestamp with time zone,
  ADD COLUMN IF NOT EXISTS trial_end_date timestamp with time zone,
  ADD COLUMN IF NOT EXISTS has_used_trial boolean NOT NULL DEFAULT false;

-- Atualizar função handle_new_user para ativar trial automaticamente
...
BEGIN
  ...
  -- Insert profile with 7-day premium trial
  INSERT INTO public.profiles (
    id, xp, level, level_title, avatar_icon, streak, total_income, total_expenses, 
    language, locale, currency,
    subscription_plan, trial_start_date, trial_end_date, has_used_trial
  )
  VALUES (
    NEW.id, 0, 1, 'Novice Saver', '🎮', 0, 0, 0, 
    user_language, user_language, user_currency,
    'PREMIUM', trial_start, trial_end, true
  );
