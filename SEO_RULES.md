...
**New Public Page:**
1. Add route to `src/lib/routeConfig.ts` → `INDEXABLE_ROUTES`
2. Add to `supabase/functions/sitemap/index.ts` → `STATIC_INDEXABLE_PAGES`

**New Blog Article:**
1. Add article to `src/lib/blogData.ts` with **mandatory** `articleType` field
2. Add to `supabase/functions/sitemap/index.ts` → `BLOG_ARTICLES`

**New Author:**
1. Add author to `src/lib/authorData.ts`
2. Add to `supabase/functions/sitemap/index.ts` → `AUTHORS`

---

## 2️⃣ Article Classification (MANDATORY)
...
## 4️⃣ Internal Linking Rules

### Satellite/Long-tail → Pillar:
...
- ✅ Absolute canonical URL (`https://moneyquest.app.br/...`)
- ✅ Indexing enabled (no noindex for public pages)
- ✅ Breadcrumb JSON-LD (when applicable)
- ✅ Article JSON-LD (for blog articles)
- ✅ Author JSON-LD (for author pages)
...
- ✅ MUST be applied automatically to any new content
